...
# ============================================================
# MINING MANAGER v11.2 (Fix: Sudo Command Execution)
# ============================================================
...
#!${TERMUX_BASH}
# WATCHDOG SCRIPT (v11.2) - Generated by mining_manager.sh
ENV_FILE="${ENV_FILE}"
MODE_FILE="${MODE_FILE}"
...
if [ ! -f "\$ENV_FILE" ]; then
    echo "\$(date): CRITICAL - Config file not found at \$ENV_FILE. Exiting." >> "\$LOG_FILE"
    exit 1
fi
source "\$ENV_FILE"
if [ -z "\$CPU_BIN" ] || [ -z "\$CPU_SERVER" ] || [ -z "\$CPU_WALLET" ]; then
    echo "\$(date): CRITICAL - Essential variables are not set in the config. Exiting." >> "\$LOG_FILE"
    exit 1
fi
...
if [ ! -x "\$CPU_BIN" ]; then
    echo "\$(date): CRITICAL - Miner binary not found or not executable at \$CPU_BIN. Exiting." >> "\$LOG_FILE"
    exit 1
fi
while true; do
...
start_watchdog() {
    ...
    # FIX: Simplified and more reliable sudo command.
    ...
    if [ -n "$PID" ]; then
        # FIX: Use sudo tee to write the PID file reliably.
...
stop_all() {
    ...
    # FIX: Use sudo to kill processes started by sudo.
    ...
    sudo pkill -f xmrig
    sudo rm -f "$WATCHDOG_PID_FILE" "$MINER_PID_FILE"
    echo "Done."
}
...
show_status() {
    ...
    if [ -f "$MINER_PID_FILE" ] && kill -0 $(cat "$MINER_PID_FILE") 2>/dev/null; then echo -e "Miner:    ${GREEN}RUNNING${NC}"; else echo -e "Miner:    ${YELLOW}STOPPED${NC}"; fi
    echo "---------------------------"
}
